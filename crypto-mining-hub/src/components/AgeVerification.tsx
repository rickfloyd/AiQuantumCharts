...
const AgeVerification: React.FC<AgeVerificationProps> = ({ onVerified }) => {
  ...
  const handleVerification = () => {
    ...
    if (age < 18) {
      setError('You must be 18 or older to access this crypto mining platform');
      return;
    }

    // Store verification in localStorage
    localStorage.setItem('age_verified_crypto', 'true');
    localStorage.setItem('age_verified_crypto_date', new Date().toISOString());
    onVerified();
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-95 flex items-center justify-center z-50">
      <div className="bg-gray-900 rounded-lg p-8 max-w-md mx-4 text-center border-4 border-hot-pink shadow-2xl shadow-hot-pink/50">
        ...
        <p className="text-gray-300 mb-6 text-lg">
          This crypto mining platform contains financial content and tools intended for adults only. 
        ...
        <p className="text-xs text-hot-pink mt-4 font-semibold animate-pulse">
          ⚠️ By proceeding, you confirm that you are of legal age and understand the financial risks of cryptocurrency mining.
        </p>
        
        <p className="text-xs text-hot-pink mt-2 font-bold">
          💰 Crypto mining involves financial risk and electricity costs
        </p>
      </div>
    </div>
  );
